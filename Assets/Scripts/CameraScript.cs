...
public class CameraScript : MonoBehaviour//make sure this always matches on unity
{
    ...
    void LateUpdate()
    {
        this.transform.position = new Vector3(target.transform.position.x, this.transform.position.y,this.transform.position.z); //update x based on character movement 
    }
}
